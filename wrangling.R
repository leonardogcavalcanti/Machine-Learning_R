...
dataset <- dataset %>% rename(SEXO = CS_SEXO,
                              IDADE = NU_IDADE_N,
                              SRAG = CLASSI_FIN)
...
# Obs3.: Variável "SRAG" tem valores negativos, acredito que é melhor apagar como Outliers.
...
counts_sexo <- table(dataset$SEXO)
barplot(counts_sexo,
        main = "Gêneros",
        xlab = "Gêneros",
        ylab = "Fequência",
...
# SRAG
summary(dataset$SRAG)
counts_srag <- table(dataset$SRAG)
barplot(counts_srag)
counts_srag 
...
# Variável IDADE
summary(dataset$IDADE) # anomalias de idades negativas e idades acima de 120 anos de idade
hist(dataset$IDADE,
     main = "Idades",
     xlab = "Idades",
     ylab = "Frequência",
...
counts_sexo_2 <- table(dataset$SEXO)
...
dataset[dataset$IDADE < 0 | dataset$IDADE > 111,]$IDADE <- median(dataset$IDADE)
# buscando dados com essas características 
dataset[dataset$IDADE < 0 | dataset$IDADE > 111,]
# Revendo
summary(dataset$IDADE)
table(dataset$IDADE)
...
dataset$SRAG <- as.factor(dataset$SRAG)
str(dataset)
# fazendo table para novo barplot
counts_srag_2 <- table(dataset$SRAG)
barplot(counts_srag_2)
counts_srag_2
# Obs.: Diagnóstico final do caso 
#1-SRAG por influenza
#2-SRAG por outro vírus respiratório
...
dataset <- mutate(dataset, INFANTIL = ifelse(IDADE == "INFANTIL",1,0))
dataset <- mutate(dataset, JOVEM = ifelse(IDADE == "JOVEM",1,0))
dataset <- mutate(dataset, ADULTO = ifelse(IDADE == "ADULTO",1,0))
dataset <- mutate(dataset, IDOSO = ifelse(IDADE == "IDOSO", 1,0))

# Apagando variável anterior
dataset <- select(dataset, everything(), -IDADE)  

# Realocando as variáveis
dataset <- dataset %>% 
  relocate(INFANTIL, JOVEM, ADULTO, IDOSO, .after = SEXO)
...
# Criando One Hot Encoder 
dataset <- mutate(dataset, INFLUENZA = ifelse(SRAG == "SRAG_influenza", 1,0))
dataset <- mutate(dataset, OUTROS_VIRUS = ifelse(SRAG == "SRAG_outros_virus", 1,0))
dataset <- mutate(dataset, OUTROS_AGENTES = ifelse(SRAG == "SRAG_outros_agente_etiologico",1,0))
dataset <- mutate(dataset, NAO_ESPECIFICADO = ifelse(SRAG == "SRAG_nao_especificado" , 1,0))
dataset <- mutate(dataset, COVID_19 = ifelse(SRAG == "SRAG_covid_19" , 1,0))

# Apagando variável anterior
dataset <- select(dataset, everything(), -SRAG)  
str(dataset)
# Realocando as variáveis
dataset <- dataset %>% 
...
dataset_full <- read.csv("dataset_tcc.csv", 
                         sep = ",", 
                         na.strings = "", 
                         stringsAsFactors = T)
...
metricas_resample <- mdl_fit_resample_logistic %>% 
  collect_metrics()
assess_res <- collect_predictions(mdl_fit_resample_logistic)
